...
(function() {
  ...
  angular.module('BlurAdmin.pages.workers')
    .controller('WorkersGroupCtrl', WorkersGroupCtrl);

  /** @ngInject */
  function WorkersGroupCtrl($scope, $filter, editableOptions, editableThemes, WorkerService) {
    var ctrl = this;
    ...
    this.callServer = function callServer(tableState) {
      ...
      WorkerService.getgrouplist(start, number, tableState).then(function(result) {
    ...
    this.removeitem = function(row) {
      ...
      layer.confirm('are you sure to delete?', function(index) {
        layer.load(1);
        WorkerService.deletegroup(row.id).then(function(result) {
